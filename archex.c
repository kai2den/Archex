...
int main(int argc, char *argv[]) {
    ...
    verbose = 0;
    uint8_t version = 0x01; // Default version
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-i") == 0 && i + 1 < argc) input_file = argv[++i];
            ...
            char *endptr;
            long val = strtol(argv[++i], &endptr, 16); // Parse version in hex
    ...
    if (!input_file) {
        fprintf(stderr, "Usage: %s -i <input_file> [-o <output_dir>] [-v [0|1|2]] [-vn <version>]\n", argv[0]);
        fprintf(stderr, "       -vn: Specify version number in hex (e.g., 0x02)\n");
        return 1;
    }
    ...
    // Verify the archive version
    if (data[4] != version) {
        log_error("Unsupported version (expected 0x%02x, got 0x%02x)", version, data[4]);
        free(data);
        fclose(log_fp);
        fclose(report_fp);
        return 1;
