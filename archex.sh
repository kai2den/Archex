...
show_help() {
    ...
    echo "  set vn <version>    - Set the version number in hex (e.g., 0x02)"
    ...
    case $cmd in
        ...
        set)
            case $arg1 in
                ...
                vn)
                    if [ -z "$arg2" ]; then
                        echo "Error: Please specify a version number in hex (e.g., set vn 0x02)"
                    else
                        VERSION_NUMBER="$arg2"  # Set version number
        ...
        run)
            ...
            # Build and execute the archex command
            command="./archex -i $INPUT_FILE -o $OUTPUT_DIR -v $VERBOSE_MODE -vn $VERSION_NUMBER"
